...
public class FormStack : List<IStackedForm>, IDisposable, ISuspendable, ICompactable
{
    ...
    public class StackId : IStackId
    {
        ...
        {
            string strTemp = FormType.TypeToReadable();
        ...
        public virtual bool Equals(IStackId other)
        {
            ...
            else if (ReferenceEquals(other.GetType(), typeof(StackId)))
            {   // the other object type is directly StackId, just compare values
                result = FormType.Equals(other.FormType);
            }
            else
    ...
    public class EventFormActivatedEventArgs : EventFormStackItemActionArgs
    {
        ...
        /// <see cref="IStackedForm "/> interface. </param>
    ...
    public event EventHandler<EventFormStackItemActionArgs> EventFormAction;
    ...
    {
        ExecutionState = ExecState.RunsOwnMsgPump;
        for (; ; )
        {
            if (!IsSuspended)
            {
        ...
        ExecutionState = ExecState.NotRunning;
    }
    ...
    public virtual void RunApplication()
    {
        ...
        try
        {
            ExecutionState = ExecState.RunsAppRun;
            Application.Run();
        }
        finally
        {
    ...
    /// is actually a wrapper around a Form - either a <see cref="StackedFormWrapper{TForm}"/> or
    /// <see cref="MainStackedFormWrapper{TForm}"/> or any type derived from these types. </summary>
    ///
        ...
        bool bModal, IStackId id, Type[] argTypes, object[] args)
    {
        ...
        {
            IStackedForm iNewTop;
            IStackedForm iOldTop = TopStackedForm;
            ...
            {  // now show modal
                ...
                if (!iNewTop.PermitsCaching)
                {
                    ...
                    RemoveUnusedForms(item => ReferenceEquals(item, iNewTop));
                }
                RaiseEventFormAction(new EventFormActivatedEventArgs(TopStackedForm));
            }
            else
            {
                bool bEquals = Equals(iOldTop, iNewTop);
    ...
    {
        EventFormAction?.Invoke(this, args);
    }
    ...
    protected IStackedForm CreateAndPreload(IStackId id, Type[] argTypes, object[] args)
    {
        ...
        lock (Locker)
        {
            if (null != (iForm = CreateFormInstance(id, argTypes, args)))
            {
                if (Preload(iForm))
                {  // add it to the cache
                    FormList.Add(iForm);
    ...
    private void On_evStackItemClosed(object sender, EventFormStackItemClosedArgs e)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (!ReferenceEquals(sender, top.MyForm))
                {
                    senderIsWrong = true;
                }
                if (senderIsWrong)
            ...
            { // I cannot do this for a Form that is still in a modal state
                RemoveUnusedForms(item => ReferenceEquals(item, popped));
            }
        }
    }
